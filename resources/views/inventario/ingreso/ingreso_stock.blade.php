...
@section('title-content' , 'Entrada de productos')
@section('content')
...
<div class="page-header">
    <div class="row align-items-center">
        <div class="col-auto">
            <h2 class="page-title">Entrada de productos</h2>
        </div>
    </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12 mb-2" id="idProducto_fk_has-error">
                            ...
                            <select id="idProducto_fk" class="form-control select-destin" onchange="validaterRegisterProduct()" style="width: 100%">
                                ...
                                <option value="{{$item->idProducto}}">{{$item->nombre}} {{$item->descripcion == null ?'' : ($item->descripcion)}}</option>
                                @endforeach
                            </select>
                        </div>
                                    ...
                                    <optgroup label="{{mb_strtoupper($item->lugar)}}">
                                        @foreach ($tipos as $item2)
                                            @if ($item2->area_fk == $item->idArea)
                                                <option value="{{$item2->idTipo}}">{{mb_strtoupper($item2->nombre)}}</option>
                                            @endif
                                        @endforeach
                                    </optgroup>
                                @endforeach
...
<script>
    ...
    document.querySelector('#btnCreateRegister').addEventListener('click', ()=>{
        ...
        var lote = validaterRegister('lote')
        var fecha = validaterRegister('fecha_cadu')

        if(area_fk == "success" && producto == "success" && lote == "success" && fecha == "success"){
            ...
            .then(response => response.json()).then(data => {
                console.log(data)
                if(data){
                    if(data.success){ toastr.success(data.success);  window.location.href = "/ingreso/productos";}
                    if(data.error){toastr.error(data.error); $("#btnCreateRegister").attr('disabled', false).text("").append(icon_btn , spin_btn);}
                }
            })
            .catch(error => {
