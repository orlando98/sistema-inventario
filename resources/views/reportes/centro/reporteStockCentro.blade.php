...
@section('title-content' , 'Reporte stock (Centro)')
@section('content')
<!-- Page title -->
...
<script>
    ...
    function searchFilters(){
        ...
        if(area == "success"){
            ...
            fetch(`/filtro/reporte/stock/centro/parametros/${area}`)
            .then(response => response.json()).then(data => {
                if(data.length > 0){
                    ...
                    var btnPDF = document.getElementById("btn_pdf");
                        btnPDF.setAttribute('href', `/PDF/reporte/stock/centro/${area}`)
